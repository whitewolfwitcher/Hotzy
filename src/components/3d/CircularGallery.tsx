...
class App {
  ...
  onCanvasClick(e: MouseEvent) {
    ...
    // Simple click detection - check if clicking near center
    if (Math.abs(x) < 0.3 && Math.abs(y) < 0.3 && this.medias && this.onItemClick) {
      // Find the media closest to center with explicit type annotations
      let closestImage: string = '';
      let closestText: string = '';
      ...
      this.medias.forEach((media: Media) => {
        ...
        if (distance < minDistance) {
          ...
          closestImage = media.image;
          closestText = media.text;
        }
      });
      
      if (closestImage && minDistance < 3) {
        this.onItemClick(closestImage, closestText);
      }
    }
  }
